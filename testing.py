...
stream = urllib.request.urlopen('http://192.168.2.4:8000/video_feed')
total_bytes = b''
while True:
    ...
    if not b == -1:
        ...
        cv2.imshow('Feed',img) # display image while receiving data
        if cv2.waitKey(1) == ord("q"): # if user hit esc            
            break
